# -*- coding: utf-8 -*-
"""
Created on Thu Aug 20 16:31:56 2020

@author: Jieyun Hu
"""
# data fustion strategy 1:
# merge multiple motion data and feed into models
# Only merge motions data
...
class models():
    ...
    def read_h5(self, path_array):
        ...
        '''      
    #merge different motion data (All should with the same dimension and same y) before feed into models
    def merge(self, data_array):
        merge_X = data_array[0][0] # initialize with the first dataset
        print(merge_X.shape)
        y = data_array[0][1]
        if len(data_array)>1:
            for i, data in enumerate(data_array):
                if i != 0:
                    merge_X = np.concatenate((merge_X, data[0]), axis = -1)
        print(merge_X.shape)
        self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(merge_X, y, test_size=0.2, random_state = 1)           
        print(self.x_train.shape)
...
if __name__ == "__main__":
    ...
    paths = ["./bag.h5", "./hand.h5", "./hip.h5","./torso.h5"]
    motion = models()
    print("read h5 file....")
    data_array = motion.read_h5(paths)
    motion.merge(data_array)

# -*- coding: utf-8 -*-
"""
Created on Thu Aug 20 21:24:39 2020

@author: Jieyun Hu
"""
# data fusion strategy 2
# merge the data in modeling
...
class models():
    ...
    def create_cnn(self, input_shape, K):
    ...
    def merge_models(self,n):
        input_shape = np.expand_dims(self.split[0], -1)[0].shape
        K = len(set(self.split[-2]))
        print(input_shape)
        cnns = [] # save all cnn models
        for i in range(n):
            cnn_i = self.create_cnn(input_shape,K)
            cnns.append(cnn_i)
...
if __name__ == "__main__":
    ...
    motion.merge_models(len(paths))
    motion.draw()
    motion.con_matrix(len(paths))
    '''

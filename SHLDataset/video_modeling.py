# -*- coding: utf-8 -*-
"""
Created on Wed Aug 19 11:32:42 2020
...
class models():
    ...
    def cnn_model(self):
        K = len(set(self.y_train)) # no 0 in label
        print(K)
        #print(K)
        #K = 12
        ...
        self.r = self.model.fit(self.x_train, self.y_train, validation_data = (self.x_test, self.y_test), epochs = 100, batch_size = 32 )
        print(self.model.summary())
        # It is better than using keras do the splitting!!
        return self.r
    
    #model for segment_model
   
    def cnn_segment_model(self):
        K = len(set(self.y_train)) # no 0 in label
        print(K)
        print(self.x_train.shape)
        self.x_train = np.expand_dims(self.x_train, -1)
...
if __name__ == "__main__":
    model_name = "cnn" 
    #path = "./video.h5"
    path = "./image_for_fusion.h5" #segmentation
    video = models(path)
    print("read h5 file....")
    video.read_h5()
    
    if model_name == "cnn":
        video.cnn_segment_model()
    video.draw()
    video.con_matrix()